  ...
  return (
    <div className="min-h-screen bg-white dark:bg-slate-900 transition-colors py-12">
      <div className="container mx-auto px-6 md:px-12">
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-4 animate-slide-down">
        ...
        {events.length === 0 ? (
          <div className="text-center py-16 bg-slate-50 dark:bg-slate-800 rounded-2xl animate-on-scroll">
            <p className="text-xl text-slate-500 dark:text-slate-400">No events found. Create one!</p>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {events.map((e, i) => (
              <div key={e._id} className={`animate-on-scroll delay-${(i % 6 + 1) * 100}`}>
                <EventCard event={e} currentUserId={currentUserId} onDelete={handleEventDeleted} />
              </div>
            ))}
          </div>
        )}
